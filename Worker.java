        ...
        if(serverUpdate){
            ...
            try {
                ...
                switch (receivedMsg[0].toString()) {
                    ...
                    case "REGISTER": //    FROM CLIENT
                        ...
                        try {
                            logSem.acquire();
                            Server.logger.logEvent("Message Received From: " + receivedMsg[2] +". IP:"+ receivedMsg[3] + ":" + receivedMsg[4]);
                            logSem.release();
                            ...
                            if (nameExists.equals(false)) {
                                ...
                                for (int i = 0; i < receivedMsg.length; i++) {
                                    System.out.print(receivedMsg[i].toString()+"|");
                                }
                                System.out.println();
                                // Add user to DB
                        ...
                        try {
                            ...
                            if (nameExists.equals(true)) {
                                ...
                                if(Server.isServing.get()) {
                                    ...
                                    System.out.println("De-Register: User <" + receivedMsg[2].toString() + "> has been de-registered...");
                                }
                                else{
                                    db_int.deleteUser(receivedMsg[1].toString());
                    ...
                    case "REGISTERED": //    FROM SERVING SERVER
                        ...
                        try {
                            ...
                            // Save user into db
                            db_int.createNewUserAccount(receivedMsg[2].toString(), userFullAddress);
                            System.out.println("[From serving server] Registered: User <" + receivedMsg[2].toString() + "> registered and received from: " + clientIP+":"+clientPort);
                            db_int.disconnect();
                            logSem.acquire();
                    ...
                    case "REGISTER-DENIED": //    FROM SERVING SERVER
                        ...
                        System.out.println("[From serving server] Register-Denied: User <" + receivedMsg[2].toString() + "> already exists...");
                        try {
                            logSem.acquire();
                    ...
                    case "UPDATE": //    FROM CLIENT
                        ...
                        try {
                            ...
                            if (nameExists.equals(true)) {
                                System.out.println("UPDATE-CONFIRMED: updating user <" + userInfo[0] + "> socket address.");
                              //  System.out.println("IP: " + receivedMsg[3].toString() + " Socket#: " + receivedMsg[4].toString());

                                logSem.acquire();
                                Server.logger.logEvent("UPDATE-CONFIRMED: updating user <" + userInfo[0] + "> socket address."+"IP: " + receivedMsg[3].toString() + " Socket#: " + receivedMsg[4].toString());
                                logSem.release();
                    ...
                    case "UPDATE-CONFIRMED": //    FROM SERVING SERVER
                        ...
                        try {
                            ...
                            if (nameExists.equals(true)) {
                                System.out.println("[From serving server] updating user <" + userInfo[0] + "> address");
                               // System.out.println("IP: " + receivedMsg[3].toString() + " Socket#: " + receivedMsg[4].toString());
                                ...
                                System.out.println("[From serving server] update request received but could not process: user <" + receivedMsg[2] + "> does not exist.");
                    ...
                    case "SUBJECTS": //    FROM CLIENT
                        try {
                            ...
                            if (nameExists.equals(true) && acceptedSubjectList.size() != 0) {
                                ...
                                logSem.acquire();
                                Server.logger.logEvent("Subject List: " + stringList.substring(0, stringList.length() - 1));
                                logSem.release();
                            } else {
                    ...
                    case "SUBJECTS-UPDATED": //    SERVING SERVER
                        ...
                        db_int.disconnect();
                        System.out.println("[From serving server] Subjects-Updated: User <" + receivedMsg[2].toString() + "> updated " + subjectList.size() + " subjects: " + stringList.substring(0, stringList.length() - 1));
                        try {
                            logSem.acquire();
                           ...
                           // System.out.println(receivedMsg[2].toString());
                            ...
                            if (nameExists && usersSubscribed.size() > 0 && usersSubscribed != null && subjectId != -1) {
                                ...
                                for (int i = 0; i < usersSubscribed.size(); i++) {
                                    ...
                                    replyMsgClient[1] = receivedMsg[2];
                                    ...
                                    if (replyMsgClient != null) {
                                        if(!name.equalsIgnoreCase(receivedMsg[2].toString())) {
                                ...
                                String sender = receivedMsg[2].toString();
                                String fullAddress = clientIP+":"+Integer.toString(clientPort);//receivePacket.getSocketAddress().toString();
                                db_int.sendMessage(sender, subjectId, text, fullAddress);
                   ...
                   /* case "PREPARE-SWAP": //    FROM OTHER SERVER
                    ...
                    case "PREPARE-SWAP": //    FROM OTHER SERVER
                        ...
                        ServerInterrupt.resumeTimerTask();
                        Server.isServing.set(true);
                        System.out.println("Serving = true");
                        break;
                    ...
                    case "READY-SWAP": //    FROM OTHER SERVER
                        ...
                        //warn all users of change-server:
                        replyMsgClient = new Object[3];
                        replyMsgClient[0] = "CHANGE-SERVER";
                        replyMsgClient[1] = otherServerIP;
                        replyMsgClient[2] = otherServerPort;
                        ArrayList<String[]> allUserInfo = db_int.getAllUsers();
                        String temp[] =null;
                        String fulladdr = null;
                        String[] addressSplit = null; //fullAddress.split(":"); // IP[0] and Port[1]
                        db_int.disconnect();


                        for(int i=0;i<allUserInfo.size();i++){
                            temp = allUserInfo.get(i);
                            fulladdr = temp[1];
                            addressSplit = fulladdr.split(":");
                        ...
                        ServerInterrupt.pauseTimerTask();
                        Server.isServing.set(false);
                        System.out.println("Serving = false");
                        try {
                            logSem.acquire();
                            Server.logger.logEvent("Warning message sent to all registered users of server change.");
            ...
            if (replyMsgClient != null) {
                try {
                    ...
                    logSem.acquire();
                    Server.logger.logEvent("sending msg "+replyMsgClient[0]+ " to client: "+ clientIP+":"+clientPort);
                    logSem.release();
            ...
            if(replyMsgServer != null){
                try {
                    sendMessage(replyMsgServer,otherServerIP,otherServerPort);
                    logSem.acquire();
                    Server.logger.logEvent("sending msg "+replyMsgServer[0]+ " to other server: "+ otherServerIP+":"+otherServerPort);
                    logSem.release();
                } catch (UnknownHostException | InterruptedException e) {
                    e.printStackTrace();
                }
